    ...
    extends TopologyObject
{
    ...
    {
        if (!(_path == null)) {
            final TopologyValue _value18 = currentNode.createValue("path");
            _value18 .setSampleValue(_path.toString());
            if (__isIdentity("path")) {
                _value18 .setIsIdentity(true);
            }
        }
        if (!(_protocol == null)) {
            final TopologyValue _value19 = currentNode.createValue("protocol");
            _value19 .setSampleValue(_protocol.toString());
            if (__isIdentity("protocol")) {
                _value19 .setIsIdentity(true);
            }
        }
        if (!(_name == null)) {
            final TopologyValue _value20 = currentNode.createValue("name");
            _value20 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value20 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node21 = currentNode.createNode("monitoringAgent");
            ...
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                ...
                _node21 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node21 .setTypeHint(_monitoringAgent.__getNodeName());
                _node21 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node21, unitService, submitted);
            }
        }
    }
