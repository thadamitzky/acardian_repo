    ...
    extends TopologyObject
{
        ...
        throws TopologyException
    {
        ...
        if (!(_port == null)) {
            final TopologyValue _value23 = currentNode.createValue("port");
            _value23 .setSampleValue(_port);
            if (__isIdentity("port")) {
                _value23 .setIsIdentity(true);
            }
        }
        if (!(_name == null)) {
            final TopologyValue _value24 = currentNode.createValue("name");
            _value24 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value24 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node25 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node25 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node25 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node25 .setTypeHint(_monitoringAgent.__getNodeName());
                _node25 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node25, unitService, submitted);
            }
        }
    }
