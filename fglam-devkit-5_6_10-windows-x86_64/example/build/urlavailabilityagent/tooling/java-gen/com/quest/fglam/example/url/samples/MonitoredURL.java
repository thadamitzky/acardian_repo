    ...
    extends CollectionModelInstance
{
        ...
        throws TopologyException
    {
        ...
        if (!(_urlPath == null)) {
            final TopologyNode _node9 = currentNode.createNode("urlPath");
            ...
            if (submitted.contains(_urlPath.__getNodeId())) {
                ...
                _node9 .setRefId(_urlPath.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node9 .setTypeHint(_urlPath.__getNodeName());
                _node9 .setId(_urlPath.__getNodeId().toString());
                _urlPath.addToNodeTree(_node9, unitService, submitted);
            }
        }
        if (!(_urlServerInstance == null)) {
            final TopologyNode _node10 = currentNode.createNode("urlServerInstance");
            if (__isIdentity("urlServerInstance")) {
                _node10 .setIsIdentity(true);
            }
            if (submitted.contains(_urlServerInstance.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node10 .setRefId(_urlServerInstance.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node10 .setTypeHint(_urlServerInstance.__getNodeName());
                _node10 .setId(_urlServerInstance.__getNodeId().toString());
                _urlServerInstance.addToNodeTree(_node10, unitService, submitted);
            }
        }
        if (!(_monitoredHost == null)) {
            final TopologyNode _node11 = currentNode.createNode("monitoredHost");
            if (__isIdentity("monitoredHost")) {
                _node11 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoredHost.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node11 .setRefId(_monitoredHost.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node11 .setTypeHint(_monitoredHost.__getNodeName());
                _node11 .setId(_monitoredHost.__getNodeId().toString());
                _monitoredHost.addToNodeTree(_node11, unitService, submitted);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node12 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node12 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node12 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node12 .setTypeHint(_monitoringAgent.__getNodeName());
                _node12 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node12, unitService, submitted);
            }
        }
        if (!(_availability == null)) {
            final TopologyNode _node13 = currentNode.createNode("availability");
            _availability.submit(_node13, unitService.getUnits("percent"));
        }
        if (!(_pageGetTime == null)) {
            final TopologyNode _node14 = currentNode.createNode("pageGetTime");
            _pageGetTime.submit(_node14, unitService.getUnits("millisecond"));
        }
        if (!(_responseTime == null)) {
            final TopologyNode _node15 = currentNode.createNode("responseTime");
            _responseTime.submit(_node15, unitService.getUnits("millisecond"));
        }
        if (!(_errorCode == null)) {
            final TopologyValue _value16 = currentNode.createValue("errorCode");
            _value16 .setSampleValue(_errorCode);
            if (__isIdentity("errorCode")) {
                _value16 .setIsIdentity(true);
            }
        }
        if (!(_url == null)) {
            final TopologyValue _value17 = currentNode.createValue("url");
            _value17 .setSampleValue(_url.toString());
            if (__isIdentity("url")) {
     ...
     * 
     * @return
     *     The current value of the 'errorCode' property.
     */
    ...
    /**
     * Returns the current value of the 'url' property.
     * 
     * @return
     *     The current value of the 'url' property.
     */
    ...
    /**
     * Sets the current value of the 'url' property.
     * 
     */
