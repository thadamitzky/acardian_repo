    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_interfaces == null)) {
            for (TopologyObject list_element: _interfaces) {
                final TopologyNode _node173 = currentNode.createNode("interfaces");
                if (__isIdentity("interfaces")) {
                    _node173 .setIsIdentity(true);
                }
                if (_replace_interfaces) {
                    _node173 .setReplace(true);
                }
                _replace_interfaces = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node173 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node173 .setTypeHint(list_element.__getNodeName());
                    _node173 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node173, unitService, submitted);
                }
            }
        }
        if (!(_utilization == null)) {
            final TopologyNode _node174 = currentNode.createNode("utilization");
            _utilization.submit(_node174, unitService.getUnits("percent"));
        }
        if (!(_packetsSent == null)) {
            final TopologyNode _node175 = currentNode.createNode("packetsSent");
            _packetsSent.submit(_node175, unitService.getUnits("count/second"));
        }
        if (!(_packetsReceived == null)) {
            final TopologyNode _node176 = currentNode.createNode("packetsReceived");
            _packetsReceived.submit(_node176, unitService.getUnits("count/second"));
        }
        if (!(_sendRate == null)) {
            final TopologyNode _node177 = currentNode.createNode("sendRate");
            _sendRate.submit(_node177, unitService.getUnits("bit/second"));
        }
        if (!(_receiveRate == null)) {
            final TopologyNode _node178 = currentNode.createNode("receiveRate");
            _receiveRate.submit(_node178, unitService.getUnits("bit/second"));
        }
        if (!(_transferRate == null)) {
            final TopologyNode _node179 = currentNode.createNode("transferRate");
            _transferRate.submit(_node179, unitService.getUnits("bit/second"));
        }
    }
