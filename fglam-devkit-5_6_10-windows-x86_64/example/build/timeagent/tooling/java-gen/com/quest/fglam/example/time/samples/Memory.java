    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_totalMemory == null)) {
            final TopologyValue _value23 = currentNode.createValue("totalMemory");
            _value23 .setSampleValue(_totalMemory, unitService.getUnits("megabyte"));
            if (__isIdentity("totalMemory")) {
                _value23 .setIsIdentity(true);
            }
        }
        if (!(_capacity == null)) {
            final TopologyNode _node24 = currentNode.createNode("capacity");
            _capacity.submit(_node24, unitService.getUnits("megabyte"));
        }
        if (!(_consumed == null)) {
            final TopologyNode _node25 = currentNode.createNode("consumed");
            _consumed.submit(_node25, unitService.getUnits("megabyte"));
        }
        if (!(_utilization == null)) {
            final TopologyNode _node26 = currentNode.createNode("utilization");
            _utilization.submit(_node26, unitService.getUnits("percent"));
        }
        if (!(_pageInRate == null)) {
            final TopologyNode _node27 = currentNode.createNode("pageInRate");
            _pageInRate.submit(_node27, unitService.getUnits("count/second"));
        }
        if (!(_pageOutRate == null)) {
            final TopologyNode _node28 = currentNode.createNode("pageOutRate");
            _pageOutRate.submit(_node28, unitService.getUnits("count/second"));
        }
        if (!(_virtualMemorySize == null)) {
            final TopologyNode _node29 = currentNode.createNode("virtualMemorySize");
            _virtualMemorySize.submit(_node29, unitService.getUnits("megabyte"));
        }
        if (!(_nonPagedKernelMemory == null)) {
            final TopologyNode _node30 = currentNode.createNode("nonPagedKernelMemory");
            _nonPagedKernelMemory.submit(_node30, unitService.getUnits("megabyte"));
        }
        if (!(_pagedKernelMemory == null)) {
            final TopologyNode _node31 = currentNode.createNode("pagedKernelMemory");
            _pagedKernelMemory.submit(_node31, unitService.getUnits("megabyte"));
        }
        if (!(_fileSystemCache == null)) {
            final TopologyNode _node32 = currentNode.createNode("fileSystemCache");
            _fileSystemCache.submit(_node32, unitService.getUnits("megabyte"));
        }
        if (!(_topMemoryProcesses == null)) {
            for (TopologyObject list_element: _topMemoryProcesses) {
                final TopologyNode _node33 = currentNode.createNode("topMemoryProcesses");
                if (__isIdentity("topMemoryProcesses")) {
                    _node33 .setIsIdentity(true);
                }
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node33 .setTypeHint(cloned.__getNodeName());
                    _node33 .setId(cloned.__getNodeId().toString());
                    cloned.addToNodeTree(_node33, unitService, submitted);
                } else {
                    // Not already submitted, send the entire thing
                    _node33 .setTypeHint(list_element.__getNodeName());
                    _node33 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node33, unitService, submitted);
                }
            }
        }
    }
