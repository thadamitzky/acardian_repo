...
public class HostBaseObject
    extends com.quest.fglam.example.time.samples.TopologyObject
{
        ...
        throws TopologyException
    {
        if (!(_detail == null)) {
            for (com.quest.fglam.example.time.samples.TopologyObject list_element: _detail) {
                final TopologyNode _node197 = currentNode.createNode("detail");
                if (__isIdentity("detail")) {
                    _node197 .setIsIdentity(true);
                }
                if (_replace_detail) {
                    _node197 .setReplace(true);
                }
                _replace_detail = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node197 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node197 .setTypeHint(list_element.__getNodeName());
                    _node197 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node197, unitService, submitted);
                }
            }
        }
        if (!(_name == null)) {
            final TopologyValue _value198 = currentNode.createValue("name");
            _value198 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value198 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node199 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node199 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node199 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node199 .setTypeHint(_monitoringAgent.__getNodeName());
                _node199 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node199, unitService, submitted);
            }
        }
    }
