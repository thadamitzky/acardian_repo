    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_type == null)) {
            final TopologyValue _value34 = currentNode.createValue("type");
            _value34 .setSampleValue(_type.toString());
            if (__isIdentity("type")) {
                _value34 .setIsIdentity(true);
            }
        }
        if (!(_name == null)) {
            final TopologyValue _value35 = currentNode.createValue("name");
            _value35 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value35 .setIsIdentity(true);
            }
        }
        if (!(_architecture == null)) {
            final TopologyValue _value36 = currentNode.createValue("architecture");
            _value36 .setSampleValue(_architecture.toString());
            if (__isIdentity("architecture")) {
                _value36 .setIsIdentity(true);
            }
        }
        if (!(_version == null)) {
            final TopologyValue _value37 = currentNode.createValue("version");
            _value37 .setSampleValue(_version.toString());
            if (__isIdentity("version")) {
                _value37 .setIsIdentity(true);
            }
        }
        if (!(_servicePackMajorVersion == null)) {
            final TopologyValue _value38 = currentNode.createValue("servicePackMajorVersion");
            _value38 .setSampleValue(_servicePackMajorVersion.toString());
            if (__isIdentity("servicePackMajorVersion")) {
                _value38 .setIsIdentity(true);
            }
        }
        if (!(_servicePackMinorVersion == null)) {
            final TopologyValue _value39 = currentNode.createValue("servicePackMinorVersion");
            _value39 .setSampleValue(_servicePackMinorVersion.toString());
            if (__isIdentity("servicePackMinorVersion")) {
                _value39 .setIsIdentity(true);
            }
        }
        if (!(_buildNumber == null)) {
            final TopologyValue _value40 = currentNode.createValue("buildNumber");
            _value40 .setSampleValue(_buildNumber.toString());
            if (__isIdentity("buildNumber")) {
                _value40 .setIsIdentity(true);
            }
        }
        if (!(_processes == null)) {
            for (TopologyObject list_element: _processes) {
                final TopologyNode _node41 = currentNode.createNode("processes");
                if (__isIdentity("processes")) {
                    _node41 .setIsIdentity(true);
                }
                if (_replace_processes) {
                    _node41 .setReplace(true);
                }
                _replace_processes = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node41 .setRefId(list_element.__getNodeId().toString());
                } else {
        ...
        if (!(_services == null)) {
            for (TopologyObject list_element: _services) {
                final TopologyNode _node42 = currentNode.createNode("services");
                if (__isIdentity("services")) {
                    _node42 .setIsIdentity(true);
                }
                if (_replace_services) {
                    _node42 .setReplace(true);
                }
                _replace_services = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node42 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node42 .setTypeHint(list_element.__getNodeName());
                    _node42 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node42, unitService, submitted);
                }
            }
        }
    }
