    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value45 = currentNode.createValue("name");
            _value45 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value45 .setIsIdentity(true);
            }
        }
        if (!(_agentVersion == null)) {
            final TopologyValue _value46 = currentNode.createValue("agentVersion");
            _value46 .setSampleValue(_agentVersion.toString());
            if (__isIdentity("agentVersion")) {
                _value46 .setIsIdentity(true);
            }
        }
        if (!(_agentName == null)) {
            final TopologyValue _value47 = currentNode.createValue("agentName");
            _value47 .setSampleValue(_agentName.toString());
            if (__isIdentity("agentName")) {
                _value47 .setIsIdentity(true);
            }
        }
        if (!(_agentInstance == null)) {
            final TopologyValue _value48 = currentNode.createValue("agentInstance");
            _value48 .setSampleValue(_agentInstance.toString());
            if (__isIdentity("agentInstance")) {
                _value48 .setIsIdentity(true);
            }
        }
        if (!(_build == null)) {
            final TopologyValue _value49 = currentNode.createValue("build");
            _value49 .setSampleValue(_build.toString());
            if (__isIdentity("build")) {
                _value49 .setIsIdentity(true);
            }
        }
        if (!(_cpuCount == null)) {
            final TopologyValue _value50 = currentNode.createValue("cpuCount");
            _value50 .setSampleValue(_cpuCount.toString());
            if (__isIdentity("cpuCount")) {
                _value50 .setIsIdentity(true);
            }
        }
        if (!(_hostName == null)) {
            final TopologyValue _value51 = currentNode.createValue("hostName");
            _value51 .setSampleValue(_hostName.toString());
            if (__isIdentity("hostName")) {
                _value51 .setIsIdentity(true);
            }
        }
        if (!(_type == null)) {
            final TopologyValue _value52 = currentNode.createValue("type");
            _value52 .setSampleValue(_type.toString());
            if (__isIdentity("type")) {
                _value52 .setIsIdentity(true);
            }
        }
        if (!(_runningStateObservation == null)) {
            final TopologyValue _value53 = currentNode.createValue("runningStateObservation");
            _value53 .setSampleValue(_runningStateObservation.getValue());
            if (__isIdentity("runningStateObservation")) {
                _value53 .setIsIdentity(true);
            }
        }
        if (!(_monitoredHost == null)) {
            final TopologyNode _node54 = currentNode.createNode("monitoredHost");
            if (__isIdentity("monitoredHost")) {
                _node54 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoredHost.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node54 .setRefId(_monitoredHost.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node54 .setTypeHint(_monitoredHost.__getNodeName());
                _node54 .setId(_monitoredHost.__getNodeId().toString());
                _monitoredHost.addToNodeTree(_node54, unitService, submitted);
            }
        }
