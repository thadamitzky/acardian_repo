    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value220 = currentNode.createValue("name");
            _value220 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value220 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node221 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node221 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node221 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node221 .setTypeHint(_monitoringAgent.__getNodeName());
                _node221 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node221, unitService, submitted);
            }
        }
    }
