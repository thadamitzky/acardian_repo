    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value217 = currentNode.createValue("name");
            _value217 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value217 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node218 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node218 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node218 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node218 .setTypeHint(_monitoringAgent.__getNodeName());
                _node218 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node218, unitService, submitted);
            }
        }
    }
