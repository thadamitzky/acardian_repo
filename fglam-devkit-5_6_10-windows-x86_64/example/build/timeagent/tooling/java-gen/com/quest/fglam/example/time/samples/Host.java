    ...
    extends CollectionModelInstance
{
    ...
    private com.quest.fglam.example.time.samples.IPAddress _primaryIpAddress;
    ...
    public void reset() {
        ...
        if (!(_ipAddresses == null)) {
            for (com.quest.fglam.example.time.samples.TopologyObject list_element: _ipAddresses) {
                list_element.reset();
            }
        }
        if (!(_running == null)) {
            for (com.quest.fglam.example.time.samples.TopologyObject list_element: _running) {
                list_element.reset();
            }
        }
        if (!(_detail == null)) {
            for (com.quest.fglam.example.time.samples.TopologyObject list_element: _detail) {
                list_element.reset();
            }
        }
        if (!(_agents == null)) {
            for (com.quest.fglam.example.time.samples.TopologyObject list_element: _agents) {
                list_element.reset();
            }
        }
        _isHidden = null;
        ...
        if (!(_virtualHosts == null)) {
            for (com.quest.fglam.example.time.samples.TopologyObject list_element: _virtualHosts) {
                list_element.reset();
            }
        }
        ...
        throws TopologyException
    {
        ...
        if (!(_systemId == null)) {
            final TopologyValue _value133 = currentNode.createValue("systemId");
            _value133 .setSampleValue(_systemId.toString());
            if (__isIdentity("systemId")) {
                _value133 .setIsIdentity(true);
            }
        }
        if (!(_name == null)) {
            final TopologyValue _value134 = currentNode.createValue("name");
            _value134 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value134 .setIsIdentity(true);
            }
        }
        if (!(_os == null)) {
            final TopologyNode _node135 = currentNode.createNode("os");
            if (__isIdentity("os")) {
                _node135 .setIsIdentity(true);
            }
            if (submitted.contains(_os.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node135 .setRefId(_os.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node135 .setTypeHint(_os.__getNodeName());
                _node135 .setId(_os.__getNodeId().toString());
                _os.addToNodeTree(_node135, unitService, submitted);
            }
        }
        if (!(_memory == null)) {
            final TopologyNode _node136 = currentNode.createNode("memory");
            if (__isIdentity("memory")) {
                _node136 .setIsIdentity(true);
            }
            if (submitted.contains(_memory.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node136 .setRefId(_memory.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node136 .setTypeHint(_memory.__getNodeName());
                _node136 .setId(_memory.__getNodeId().toString());
                _memory.addToNodeTree(_node136, unitService, submitted);
            }
        }
        if (!(_cpus == null)) {
            final TopologyNode _node137 = currentNode.createNode("cpus");
            if (__isIdentity("cpus")) {
                _node137 .setIsIdentity(true);
            }
            if (submitted.contains(_cpus.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node137 .setRefId(_cpus.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node137 .setTypeHint(_cpus.__getNodeName());
                _node137 .setId(_cpus.__getNodeId().toString());
                _cpus.addToNodeTree(_node137, unitService, submitted);
            }
        }
        if (!(_storage == null)) {
            final TopologyNode _node138 = currentNode.createNode("storage");
            if (__isIdentity("storage")) {
                _node138 .setIsIdentity(true);
            }
            if (submitted.contains(_storage.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node138 .setRefId(_storage.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node138 .setTypeHint(_storage.__getNodeName());
                _node138 .setId(_storage.__getNodeId().toString());
                _storage.addToNodeTree(_node138, unitService, submitted);
            }
        }
        if (!(_network == null)) {
            final TopologyNode _node139 = currentNode.createNode("network");
            if (__isIdentity("network")) {
                _node139 .setIsIdentity(true);
            }
            if (submitted.contains(_network.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node139 .setRefId(_network.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node139 .setTypeHint(_network.__getNodeName());
                _node139 .setId(_network.__getNodeId().toString());
                _network.addToNodeTree(_node139, unitService, submitted);
            }
        }
        if (!(_ipAddresses == null)) {
            for (com.quest.fglam.example.time.samples.TopologyObject list_element: _ipAddresses) {
                final TopologyNode _node140 = currentNode.createNode("ipAddresses");
                if (__isIdentity("ipAddresses")) {
                    _node140 .setIsIdentity(true);
                }
                if (_replace_ipAddresses) {
                    _node140 .setReplace(true);
                }
                _replace_ipAddresses = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node140 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node140 .setTypeHint(list_element.__getNodeName());
                    _node140 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node140, unitService, submitted);
                }
            }
        }
        if (!(_running == null)) {
            for (com.quest.fglam.example.time.samples.TopologyObject list_element: _running) {
                final TopologyNode _node141 = currentNode.createNode("running");
                if (__isIdentity("running")) {
                    _node141 .setIsIdentity(true);
                }
                if (_replace_running) {
                    _node141 .setReplace(true);
                }
                _replace_running = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node141 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node141 .setTypeHint(list_element.__getNodeName());
                    _node141 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node141, unitService, submitted);
                }
            }
        }
        if (!(_detail == null)) {
            for (com.quest.fglam.example.time.samples.TopologyObject list_element: _detail) {
                final TopologyNode _node142 = currentNode.createNode("detail");
                if (__isIdentity("detail")) {
                    _node142 .setIsIdentity(true);
                }
                if (_replace_detail) {
                    _node142 .setReplace(true);
                }
                _replace_detail = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node142 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node142 .setTypeHint(list_element.__getNodeName());
                    _node142 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node142, unitService, submitted);
                }
            }
        }
        if (!(_agents == null)) {
            for (com.quest.fglam.example.time.samples.TopologyObject list_element: _agents) {
                final TopologyNode _node143 = currentNode.createNode("agents");
                if (__isIdentity("agents")) {
                    _node143 .setIsIdentity(true);
                }
                if (_replace_agents) {
                    _node143 .setReplace(true);
                }
                _replace_agents = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node143 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node143 .setTypeHint(list_element.__getNodeName());
                    _node143 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node143, unitService, submitted);
                }
            }
        }
        if (!(_isHidden == null)) {
            final TopologyValue _value144 = currentNode.createValue("isHidden");
            _value144 .setSampleValue(_isHidden.toString());
            if (__isIdentity("isHidden")) {
                _value144 .setIsIdentity(true);
            }
        }
        if (!(_localName == null)) {
            final TopologyValue _value145 = currentNode.createValue("localName");
            _value145 .setSampleValue(_localName.toString());
            if (__isIdentity("localName")) {
                _value145 .setIsIdentity(true);
            }
        }
        if (!(_domainName == null)) {
            final TopologyValue _value146 = currentNode.createValue("domainName");
            _value146 .setSampleValue(_domainName.toString());
            if (__isIdentity("domainName")) {
                _value146 .setIsIdentity(true);
            }
        }
        if (!(_primaryIpAddress == null)) {
            final TopologyNode _node147 = currentNode.createNode("primaryIpAddress");
            if (__isIdentity("primaryIpAddress")) {
                _node147 .setIsIdentity(true);
            }
            if (submitted.contains(_primaryIpAddress.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node147 .setRefId(_primaryIpAddress.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node147 .setTypeHint(_primaryIpAddress.__getNodeName());
                _node147 .setId(_primaryIpAddress.__getNodeId().toString());
                _primaryIpAddress.addToNodeTree(_node147, unitService, submitted);
            }
        }
        if (!(_vendor == null)) {
            final TopologyValue _value148 = currentNode.createValue("vendor");
            _value148 .setSampleValue(_vendor.toString());
            if (__isIdentity("vendor")) {
                _value148 .setIsIdentity(true);
            }
        }
        if (!(_sysObjectId == null)) {
            final TopologyValue _value149 = currentNode.createValue("sysObjectId");
            _value149 .setSampleValue(_sysObjectId.toString());
            if (__isIdentity("sysObjectId")) {
                _value149 .setIsIdentity(true);
            }
        }
        if (!(_isGateway == null)) {
            final TopologyValue _value150 = currentNode.createValue("isGateway");
            _value150 .setSampleValue(_isGateway.toString());
            if (__isIdentity("isGateway")) {
                _value150 .setIsIdentity(true);
            }
        }
        if (!(_isBridge == null)) {
            final TopologyValue _value151 = currentNode.createValue("isBridge");
            _value151 .setSampleValue(_isBridge.toString());
            if (__isIdentity("isBridge")) {
                _value151 .setIsIdentity(true);
            }
        }
        if (!(_discoveryStatus == null)) {
            final TopologyValue _value152 = currentNode.createValue("discoveryStatus");
            _value152 .setSampleValue(_discoveryStatus.getValue());
        ...
        if (!(_virtualHosts == null)) {
            for (com.quest.fglam.example.time.samples.TopologyObject list_element: _virtualHosts) {
                final TopologyNode _node153 = currentNode.createNode("virtualHosts");
                if (__isIdentity("virtualHosts")) {
                    _node153 .setIsIdentity(true);
                }
                if (_replace_virtualHosts) {
                    _node153 .setReplace(true);
                }
                _replace_virtualHosts = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node153 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node153 .setTypeHint(list_element.__getNodeName());
                    _node153 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node153, unitService, submitted);
                }
            }
        }
        if (!(_virtualHostCount == null)) {
            final TopologyNode _node154 = currentNode.createNode("virtualHostCount");
            _virtualHostCount.submit(_node154, unitService.getUnits("count"));
        }
        if (!(_parentHost == null)) {
            final TopologyNode _node155 = currentNode.createNode("parentHost");
            if (__isIdentity("parentHost")) {
                _node155 .setIsIdentity(true);
            }
            if (submitted.contains(_parentHost.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node155 .setRefId(_parentHost.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node155 .setTypeHint(_parentHost.__getNodeName());
                _node155 .setId(_parentHost.__getNodeId().toString());
                _parentHost.addToNodeTree(_node155, unitService, submitted);
            }
        }
        if (!(_numProcesses == null)) {
            final TopologyNode _node156 = currentNode.createNode("numProcesses");
            _numProcesses.submit(_node156, unitService.getUnits("count"));
        }
        if (!(_runQueueLength == null)) {
            final TopologyNode _node157 = currentNode.createNode("runQueueLength");
            _runQueueLength.submit(_node157, unitService.getUnits("count"));
        }
        if (!(_interrupts == null)) {
            final TopologyNode _node158 = currentNode.createNode("interrupts");
            _interrupts.submit(_node158, unitService.getUnits("count/second"));
        }
        if (!(_contextSwitches == null)) {
            final TopologyNode _node159 = currentNode.createNode("contextSwitches");
            _contextSwitches.submit(_node159, unitService.getUnits("count/second"));
        }
        if (!(_availablePagingSpace == null)) {
            final TopologyNode _node160 = currentNode.createNode("availablePagingSpace");
            _availablePagingSpace.submit(_node160, unitService.getUnits("percent"));
        }
    }
    ...
    public boolean equals(java.lang.Object _obj) {
        if ((!(_obj == null))&&(_obj instanceof com.quest.fglam.example.time.samples.TopologyObject)) {
            final com.quest.fglam.example.time.samples.TopologyObject _otherObject = ((com.quest.fglam.example.time.samples.TopologyObject) _obj);
     ...
     */
    public com.quest.fglam.example.time.samples.IPAddress getPrimaryIpAddress() {
     ...
     * 
     */
    public void setPrimaryIpAddress(com.quest.fglam.example.time.samples.IPAddress primaryIpAddress) {
    ...
    public void initialize(CommonTopologyInfo commonTopologyInfo, List<DnsResolvedHost> resolvedHostInfo) {
        ...
        if (primaryIPAddress!= null) {
            setPrimaryIpAddress(new com.quest.fglam.example.time.samples.IPAddress(primaryIPAddress));
        }
        for (DnsResolvedHost resHost: resolvedHostInfo) {
            for (InetAddress ip_address: resHost.getResolved()) {
                final com.quest.fglam.example.time.samples.IPAddress topo_ip_address = new com.quest.fglam.example.time.samples.IPAddress(ip_address.getHostAddress());
