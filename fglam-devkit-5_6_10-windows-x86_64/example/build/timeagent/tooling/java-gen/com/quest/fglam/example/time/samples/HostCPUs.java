    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_processors == null)) {
            for (TopologyObject list_element: _processors) {
                final TopologyNode _node88 = currentNode.createNode("processors");
                if (__isIdentity("processors")) {
                    _node88 .setIsIdentity(true);
                }
                if (_replace_processors) {
                    _node88 .setReplace(true);
                }
                _replace_processors = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node88 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node88 .setTypeHint(list_element.__getNodeName());
                    _node88 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node88, unitService, submitted);
                }
            }
        }
        if (!(_percentUserTime == null)) {
            final TopologyNode _node89 = currentNode.createNode("percentUserTime");
            _percentUserTime.submit(_node89, unitService.getUnits("percent"));
        }
        if (!(_percentSystemTime == null)) {
            final TopologyNode _node90 = currentNode.createNode("percentSystemTime");
            _percentSystemTime.submit(_node90, unitService.getUnits("percent"));
        }
        if (!(_percentIdleTime == null)) {
            final TopologyNode _node91 = currentNode.createNode("percentIdleTime");
            _percentIdleTime.submit(_node91, unitService.getUnits("percent"));
        }
        if (!(_utilization == null)) {
            final TopologyNode _node92 = currentNode.createNode("utilization");
            _utilization.submit(_node92, unitService.getUnits("percent"));
        }
        if (!(_usedHz == null)) {
            final TopologyNode _node93 = currentNode.createNode("usedHz");
            _usedHz.submit(_node93, unitService.getUnits("count"));
        }
        if (!(_totalHz == null)) {
            final TopologyNode _node94 = currentNode.createNode("totalHz");
            _totalHz.submit(_node94, unitService.getUnits("count"));
        }
        if (!(_topCPUProcesses == null)) {
            for (TopologyObject list_element: _topCPUProcesses) {
                final TopologyNode _node95 = currentNode.createNode("topCPUProcesses");
                ...
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node95 .setTypeHint(cloned.__getNodeName());
                    _node95 .setId(cloned.__getNodeId().toString());
                    cloned.addToNodeTree(_node95, unitService, submitted);
                } else {
                    // Not already submitted, send the entire thing
                    _node95 .setTypeHint(list_element.__getNodeName());
                    _node95 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node95, unitService, submitted);
                }
            }
        }
    }
