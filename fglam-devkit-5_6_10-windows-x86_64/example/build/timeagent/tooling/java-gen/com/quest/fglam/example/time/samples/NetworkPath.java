    ...
    extends TopologyEdge
{
        ...
        throws TopologyException
    {
        ...
        if (!(_source == null)) {
            final TopologyNode _node171 = currentNode.createNode("source");
            ...
            if (submitted.contains(_source.__getNodeId())) {
                ...
                _node171 .setRefId(_source.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node171 .setTypeHint(_source.__getNodeName());
                _node171 .setId(_source.__getNodeId().toString());
                _source.addToNodeTree(_node171, unitService, submitted);
            }
        }
        if (!(_destination == null)) {
            final TopologyNode _node172 = currentNode.createNode("destination");
            if (__isIdentity("destination")) {
                _node172 .setIsIdentity(true);
            }
            if (submitted.contains(_destination.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node172 .setRefId(_destination.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node172 .setTypeHint(_destination.__getNodeName());
                _node172 .setId(_destination.__getNodeId().toString());
                _destination.addToNodeTree(_node172, unitService, submitted);
            }
        }
    }
