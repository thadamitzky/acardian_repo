    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value68 = currentNode.createValue("name");
            _value68 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value68 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node69 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node69 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node69 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node69 .setTypeHint(_monitoringAgent.__getNodeName());
                _node69 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node69, unitService, submitted);
            }
        }
    }
