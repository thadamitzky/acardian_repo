    ...
    extends DataObject
{
        ...
        throws TopologyException
    {
        ...
        if (!(_processId == null)) {
            final TopologyValue _value55 = currentNode.createValue("processId");
            _value55 .setSampleValue(_processId.toString());
            if (__isIdentity("processId")) {
                _value55 .setIsIdentity(true);
            }
        }
        if (!(_name == null)) {
            final TopologyValue _value56 = currentNode.createValue("name");
            _value56 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value56 .setIsIdentity(true);
            }
        }
        if (!(_userName == null)) {
            final TopologyValue _value57 = currentNode.createValue("userName");
            _value57 .setSampleValue(_userName.toString());
            if (__isIdentity("userName")) {
                _value57 .setIsIdentity(true);
            }
        }
        if (!(_command == null)) {
            final TopologyValue _value58 = currentNode.createValue("command");
            _value58 .setSampleValue(_command.toString());
