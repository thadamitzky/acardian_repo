    ...
    extends HostBaseObject
{
        ...
        throws TopologyException
    {
        ...
        if (!(_address == null)) {
            final TopologyValue _value19 = currentNode.createValue("address");
            _value19 .setSampleValue(_address.toString());
        ...
        if (!(_originatingPaths == null)) {
            for (TopologyObject list_element: _originatingPaths) {
                final TopologyNode _node20 = currentNode.createNode("originatingPaths");
                if (__isIdentity("originatingPaths")) {
                    _node20 .setIsIdentity(true);
                }
                if (_replace_originatingPaths) {
                    _node20 .setReplace(true);
                }
                ...
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node20 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node20 .setTypeHint(list_element.__getNodeName());
                    _node20 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node20, unitService, submitted);
                }
            }
        }
        if (!(_terminatingPaths == null)) {
            for (TopologyObject list_element: _terminatingPaths) {
                final TopologyNode _node21 = currentNode.createNode("terminatingPaths");
                if (__isIdentity("terminatingPaths")) {
                    _node21 .setIsIdentity(true);
                }
                if (_replace_terminatingPaths) {
                    _node21 .setReplace(true);
                }
                _replace_terminatingPaths = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node21 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node21 .setTypeHint(list_element.__getNodeName());
                    _node21 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node21, unitService, submitted);
                }
            }
        }
        if (!(_host == null)) {
            final TopologyNode _node22 = currentNode.createNode("host");
            if (__isIdentity("host")) {
                _node22 .setIsIdentity(true);
            }
            if (submitted.contains(_host.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node22 .setRefId(_host.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node22 .setTypeHint(_host.__getNodeName());
                _node22 .setId(_host.__getNodeId().toString());
                _host.addToNodeTree(_node22, unitService, submitted);
            }
        }
    }
