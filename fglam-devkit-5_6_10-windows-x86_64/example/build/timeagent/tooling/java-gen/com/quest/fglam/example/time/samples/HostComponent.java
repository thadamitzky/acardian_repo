    ...
    extends HostBaseObject
{
        ...
        throws TopologyException
    {
        ...
        if (!(_host == null)) {
            final TopologyNode _node87 = currentNode.createNode("host");
            if (__isIdentity("host")) {
                _node87 .setIsIdentity(true);
            }
            if (submitted.contains(_host.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node87 .setRefId(_host.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node87 .setTypeHint(_host.__getNodeName());
                _node87 .setId(_host.__getNodeId().toString());
                _host.addToNodeTree(_node87, unitService, submitted);
            }
        }
    }
