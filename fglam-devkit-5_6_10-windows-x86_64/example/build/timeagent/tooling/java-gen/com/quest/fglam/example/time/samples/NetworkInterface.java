    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_name == null)) {
            final TopologyValue _value96 = currentNode.createValue("name");
            _value96 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value96 .setIsIdentity(true);
            }
        }
        if (!(_ipAddress == null)) {
            final TopologyNode _node97 = currentNode.createNode("ipAddress");
            if (__isIdentity("ipAddress")) {
                _node97 .setIsIdentity(true);
            }
            if (submitted.contains(_ipAddress.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node97 .setRefId(_ipAddress.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node97 .setTypeHint(_ipAddress.__getNodeName());
                _node97 .setId(_ipAddress.__getNodeId().toString());
                _ipAddress.addToNodeTree(_node97, unitService, submitted);
            }
        }
        if (!(_macAddress == null)) {
            final TopologyValue _value98 = currentNode.createValue("macAddress");
            _value98 .setSampleValue(_macAddress.toString());
            if (__isIdentity("macAddress")) {
                _value98 .setIsIdentity(true);
            }
        }
        if (!(_interfaceIndex == null)) {
            final TopologyValue _value99 = currentNode.createValue("interfaceIndex");
            _value99 .setSampleValue(_interfaceIndex);
            if (__isIdentity("interfaceIndex")) {
                _value99 .setIsIdentity(true);
            }
        }
        if (!(_portNumber == null)) {
            final TopologyValue _value100 = currentNode.createValue("portNumber");
            _value100 .setSampleValue(_portNumber);
            if (__isIdentity("portNumber")) {
                _value100 .setIsIdentity(true);
            }
        }
        if (!(_interfaceType == null)) {
            final TopologyValue _value101 = currentNode.createValue("interfaceType");
            _value101 .setSampleValue(_interfaceType.toString());
            if (__isIdentity("interfaceType")) {
                _value101 .setIsIdentity(true);
            }
        }
        if (!(_netmask == null)) {
            final TopologyValue _value102 = currentNode.createValue("netmask");
            _value102 .setSampleValue(_netmask.toString());
            if (__isIdentity("netmask")) {
                _value102 .setIsIdentity(true);
            }
        }
        if (!(_isLayer2 == null)) {
            final TopologyValue _value103 = currentNode.createValue("isLayer2");
            _value103 .setSampleValue(_isLayer2 .toString());
            if (__isIdentity("isLayer2")) {
                _value103 .setIsIdentity(true);
            }
        }
        if (!(_networkSegment == null)) {
            final TopologyValue _value104 = currentNode.createValue("networkSegment");
            _value104 .setSampleValue(_networkSegment.toString());
            if (__isIdentity("networkSegment")) {
                _value104 .setIsIdentity(true);
            }
        }
        if (!(_layer2Segment == null)) {
            final TopologyValue _value105 = currentNode.createValue("layer2Segment");
            _value105 .setSampleValue(_layer2Segment);
            if (__isIdentity("layer2Segment")) {
                _value105 .setIsIdentity(true);
            }
        }
        if (!(_discoveryStatus == null)) {
            final TopologyValue _value106 = currentNode.createValue("discoveryStatus");
            _value106 .setSampleValue(_discoveryStatus.getValue());
        ...
        if (!(_networks == null)) {
            for (TopologyObject list_element: _networks) {
                final TopologyNode _node107 = currentNode.createNode("networks");
                if (__isIdentity("networks")) {
                    _node107 .setIsIdentity(true);
                }
                if (_replace_networks) {
                    _node107 .setReplace(true);
                }
                _replace_networks = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node107 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node107 .setTypeHint(list_element.__getNodeName());
                    _node107 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node107, unitService, submitted);
                }
            }
        }
        if (!(_bandwidth == null)) {
            final TopologyNode _node108 = currentNode.createNode("bandwidth");
            _bandwidth.submit(_node108, unitService.getUnits("bit/second"));
        }
        if (!(_utilization == null)) {
            final TopologyNode _node109 = currentNode.createNode("utilization");
            _utilization.submit(_node109, unitService.getUnits("percent"));
        }
        if (!(_packetsSent == null)) {
            final TopologyNode _node110 = currentNode.createNode("packetsSent");
            _packetsSent.submit(_node110, unitService.getUnits("count/second"));
        }
        if (!(_packetsReceived == null)) {
            final TopologyNode _node111 = currentNode.createNode("packetsReceived");
            _packetsReceived.submit(_node111, unitService.getUnits("count/second"));
        }
        if (!(_sendRate == null)) {
            final TopologyNode _node112 = currentNode.createNode("sendRate");
            _sendRate.submit(_node112, unitService.getUnits("bit/second"));
        }
        if (!(_receiveRate == null)) {
            final TopologyNode _node113 = currentNode.createNode("receiveRate");
            _receiveRate.submit(_node113, unitService.getUnits("bit/second"));
        }
        if (!(_inboundPacketsDropped == null)) {
            final TopologyNode _node114 = currentNode.createNode("inboundPacketsDropped");
            _inboundPacketsDropped.submit(_node114, unitService.getUnits("count"));
        }
        if (!(_outboundPacketsDropped == null)) {
            final TopologyNode _node115 = currentNode.createNode("outboundPacketsDropped");
            _outboundPacketsDropped.submit(_node115, unitService.getUnits("count"));
        }
        if (!(_inboundErrors == null)) {
            final TopologyNode _node116 = currentNode.createNode("inboundErrors");
            _inboundErrors.submit(_node116, unitService.getUnits("count"));
        }
        if (!(_outboundErrors == null)) {
            final TopologyNode _node117 = currentNode.createNode("outboundErrors");
            _outboundErrors.submit(_node117, unitService.getUnits("count"));
        }
        if (!(_outputQueueLength == null)) {
            final TopologyNode _node118 = currentNode.createNode("outputQueueLength");
            _outputQueueLength.submit(_node118, unitService.getUnits("count"));
        }
    }
