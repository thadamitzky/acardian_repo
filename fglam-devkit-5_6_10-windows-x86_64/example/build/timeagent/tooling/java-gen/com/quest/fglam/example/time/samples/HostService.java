    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_displayName == null)) {
            final TopologyValue _value162 = currentNode.createValue("displayName");
            _value162 .setSampleValue(_displayName.toString());
            if (__isIdentity("displayName")) {
                _value162 .setIsIdentity(true);
            }
        }
        if (!(_state == null)) {
            final TopologyValue _value163 = currentNode.createValue("state");
            _value163 .setSampleValue(_state.getValue());
            if (__isIdentity("state")) {
                _value163 .setIsIdentity(true);
            }
        }
        if (!(_process == null)) {
            final TopologyNode _node164 = currentNode.createNode("process");
            if (__isIdentity("process")) {
                _node164 .setIsIdentity(true);
            }
            if (submitted.contains(_process.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node164 .setRefId(_process.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node164 .setTypeHint(_process.__getNodeName());
                _node164 .setId(_process.__getNodeId().toString());
                _process.addToNodeTree(_node164, unitService, submitted);
            }
        }
        if (!(_pid == null)) {
            final TopologyValue _value165 = currentNode.createValue("pid");
            _value165 .setSampleValue(_pid.toString());
            if (__isIdentity("pid")) {
                _value165 .setIsIdentity(true);
            }
        }
    }
