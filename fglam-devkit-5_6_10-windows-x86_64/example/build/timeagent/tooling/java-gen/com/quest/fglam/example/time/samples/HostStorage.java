    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_physicalDisks == null)) {
            for (TopologyObject list_element: _physicalDisks) {
                final TopologyNode _node119 = currentNode.createNode("physicalDisks");
                if (__isIdentity("physicalDisks")) {
                    _node119 .setIsIdentity(true);
                }
                if (_replace_physicalDisks) {
                    _node119 .setReplace(true);
                }
                ...
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node119 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node119 .setTypeHint(list_element.__getNodeName());
                    _node119 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node119, unitService, submitted);
                }
            }
        }
        if (!(_logicalDisks == null)) {
            for (TopologyObject list_element: _logicalDisks) {
                final TopologyNode _node120 = currentNode.createNode("logicalDisks");
                if (__isIdentity("logicalDisks")) {
                    _node120 .setIsIdentity(true);
                }
                if (_replace_logicalDisks) {
                    _node120 .setReplace(true);
                }
                _replace_logicalDisks = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node120 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
        ...
        if (!(_diskUtilization == null)) {
            final TopologyNode _node121 = currentNode.createNode("diskUtilization");
            _diskUtilization.submit(_node121, unitService.getUnits("percent"));
        }
        if (!(_diskReads == null)) {
            final TopologyNode _node122 = currentNode.createNode("diskReads");
            _diskReads.submit(_node122, unitService.getUnits("count/second"));
        }
        if (!(_diskWrites == null)) {
            final TopologyNode _node123 = currentNode.createNode("diskWrites");
            _diskWrites.submit(_node123, unitService.getUnits("count/second"));
        }
        if (!(_diskBytesRead == null)) {
            final TopologyNode _node124 = currentNode.createNode("diskBytesRead");
            _diskBytesRead.submit(_node124, unitService.getUnits("kilobyte/second"));
        }
        if (!(_diskBytesWritten == null)) {
            final TopologyNode _node125 = currentNode.createNode("diskBytesWritten");
            _diskBytesWritten.submit(_node125, unitService.getUnits("kilobyte/second"));
        }
        if (!(_diskTransferRate == null)) {
            final TopologyNode _node126 = currentNode.createNode("diskTransferRate");
            _diskTransferRate.submit(_node126, unitService.getUnits("kilobyte/second"));
        }
        if (!(_spaceUsed == null)) {
            final TopologyNode _node127 = currentNode.createNode("spaceUsed");
            _spaceUsed.submit(_node127, unitService.getUnits("megabyte"));
        }
        if (!(_spaceAvailable == null)) {
            final TopologyNode _node128 = currentNode.createNode("spaceAvailable");
            _spaceAvailable.submit(_node128, unitService.getUnits("megabyte"));
        }
        if (!(_topIOProcesses == null)) {
            for (TopologyObject list_element: _topIOProcesses) {
                final TopologyNode _node129 = currentNode.createNode("topIOProcesses");
                ...
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node129 .setTypeHint(cloned.__getNodeName());
                    _node129 .setId(cloned.__getNodeId().toString());
                    cloned.addToNodeTree(_node129, unitService, submitted);
                } else {
                    // Not already submitted, send the entire thing
