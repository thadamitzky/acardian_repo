    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value215 = currentNode.createValue("name");
            _value215 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value215 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node216 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node216 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node216 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node216 .setTypeHint(_monitoringAgent.__getNodeName());
                _node216 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node216, unitService, submitted);
            }
        }
    }
