    ...
    extends DataObject
{
        ...
        throws TopologyException
    {
        ...
        if (!(_processId == null)) {
            final TopologyValue _value46 = currentNode.createValue("processId");
            _value46 .setSampleValue(_processId.toString());
            if (__isIdentity("processId")) {
                _value46 .setIsIdentity(true);
            }
        }
        if (!(_name == null)) {
            final TopologyValue _value47 = currentNode.createValue("name");
            _value47 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value47 .setIsIdentity(true);
            }
        }
        if (!(_userName == null)) {
            final TopologyValue _value48 = currentNode.createValue("userName");
            _value48 .setSampleValue(_userName.toString());
            if (__isIdentity("userName")) {
                _value48 .setIsIdentity(true);
            }
        }
        if (!(_command == null)) {
            final TopologyValue _value49 = currentNode.createValue("command");
            _value49 .setSampleValue(_command.toString());
