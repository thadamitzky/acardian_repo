    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_name == null)) {
            final TopologyValue _value87 = currentNode.createValue("name");
            _value87 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value87 .setIsIdentity(true);
            }
        }
        if (!(_ipAddress == null)) {
            final TopologyNode _node88 = currentNode.createNode("ipAddress");
            if (__isIdentity("ipAddress")) {
                _node88 .setIsIdentity(true);
            }
            if (submitted.contains(_ipAddress.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node88 .setRefId(_ipAddress.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node88 .setTypeHint(_ipAddress.__getNodeName());
                _node88 .setId(_ipAddress.__getNodeId().toString());
                _ipAddress.addToNodeTree(_node88, unitService, submitted);
            }
        }
        if (!(_macAddress == null)) {
            final TopologyValue _value89 = currentNode.createValue("macAddress");
            _value89 .setSampleValue(_macAddress.toString());
            if (__isIdentity("macAddress")) {
                _value89 .setIsIdentity(true);
            }
        }
        if (!(_interfaceIndex == null)) {
            final TopologyValue _value90 = currentNode.createValue("interfaceIndex");
            _value90 .setSampleValue(_interfaceIndex);
            if (__isIdentity("interfaceIndex")) {
                _value90 .setIsIdentity(true);
            }
        }
        if (!(_portNumber == null)) {
            final TopologyValue _value91 = currentNode.createValue("portNumber");
            _value91 .setSampleValue(_portNumber);
            if (__isIdentity("portNumber")) {
                _value91 .setIsIdentity(true);
            }
        }
        if (!(_interfaceType == null)) {
            final TopologyValue _value92 = currentNode.createValue("interfaceType");
            _value92 .setSampleValue(_interfaceType.toString());
            if (__isIdentity("interfaceType")) {
                _value92 .setIsIdentity(true);
            }
        }
        if (!(_netmask == null)) {
            final TopologyValue _value93 = currentNode.createValue("netmask");
            _value93 .setSampleValue(_netmask.toString());
            if (__isIdentity("netmask")) {
                _value93 .setIsIdentity(true);
            }
        }
        if (!(_isLayer2 == null)) {
            final TopologyValue _value94 = currentNode.createValue("isLayer2");
            _value94 .setSampleValue(_isLayer2 .toString());
            if (__isIdentity("isLayer2")) {
                _value94 .setIsIdentity(true);
            }
        }
        if (!(_networkSegment == null)) {
            final TopologyValue _value95 = currentNode.createValue("networkSegment");
            _value95 .setSampleValue(_networkSegment.toString());
            if (__isIdentity("networkSegment")) {
                _value95 .setIsIdentity(true);
            }
        }
        if (!(_layer2Segment == null)) {
            final TopologyValue _value96 = currentNode.createValue("layer2Segment");
            _value96 .setSampleValue(_layer2Segment);
            if (__isIdentity("layer2Segment")) {
                _value96 .setIsIdentity(true);
            }
        }
        if (!(_discoveryStatus == null)) {
            final TopologyValue _value97 = currentNode.createValue("discoveryStatus");
            _value97 .setSampleValue(_discoveryStatus.getValue());
        ...
        if (!(_networks == null)) {
            for (TopologyObject list_element: _networks) {
                final TopologyNode _node98 = currentNode.createNode("networks");
                if (__isIdentity("networks")) {
                    _node98 .setIsIdentity(true);
                }
                if (_replace_networks) {
                    _node98 .setReplace(true);
                }
                _replace_networks = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node98 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node98 .setTypeHint(list_element.__getNodeName());
                    _node98 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node98, unitService, submitted);
                }
            }
        }
        if (!(_bandwidth == null)) {
            final TopologyNode _node99 = currentNode.createNode("bandwidth");
            _bandwidth.submit(_node99, unitService.getUnits("bit/second"));
        }
        ...
        if (!(_packetsSent == null)) {
            final TopologyNode _node101 = currentNode.createNode("packetsSent");
            _packetsSent.submit(_node101, unitService.getUnits("count/second"));
        }
        if (!(_packetsReceived == null)) {
            final TopologyNode _node102 = currentNode.createNode("packetsReceived");
            _packetsReceived.submit(_node102, unitService.getUnits("count/second"));
        }
        if (!(_sendRate == null)) {
            final TopologyNode _node103 = currentNode.createNode("sendRate");
            _sendRate.submit(_node103, unitService.getUnits("bit/second"));
        }
        if (!(_receiveRate == null)) {
            final TopologyNode _node104 = currentNode.createNode("receiveRate");
            _receiveRate.submit(_node104, unitService.getUnits("bit/second"));
        }
        if (!(_inboundPacketsDropped == null)) {
            final TopologyNode _node105 = currentNode.createNode("inboundPacketsDropped");
            _inboundPacketsDropped.submit(_node105, unitService.getUnits("count"));
        }
        if (!(_outboundPacketsDropped == null)) {
            final TopologyNode _node106 = currentNode.createNode("outboundPacketsDropped");
            _outboundPacketsDropped.submit(_node106, unitService.getUnits("count"));
        }
        if (!(_inboundErrors == null)) {
            final TopologyNode _node107 = currentNode.createNode("inboundErrors");
            _inboundErrors.submit(_node107, unitService.getUnits("count"));
        }
        if (!(_outboundErrors == null)) {
            final TopologyNode _node108 = currentNode.createNode("outboundErrors");
            _outboundErrors.submit(_node108, unitService.getUnits("count"));
        }
        if (!(_outputQueueLength == null)) {
            final TopologyNode _node109 = currentNode.createNode("outputQueueLength");
            _outputQueueLength.submit(_node109, unitService.getUnits("count"));
        }
    }
