    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_processors == null)) {
            for (TopologyObject list_element: _processors) {
                final TopologyNode _node79 = currentNode.createNode("processors");
                if (__isIdentity("processors")) {
                    _node79 .setIsIdentity(true);
                }
                if (_replace_processors) {
                    _node79 .setReplace(true);
                }
                _replace_processors = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node79 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node79 .setTypeHint(list_element.__getNodeName());
                    _node79 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node79, unitService, submitted);
                }
            }
        }
        if (!(_percentUserTime == null)) {
            final TopologyNode _node80 = currentNode.createNode("percentUserTime");
            _percentUserTime.submit(_node80, unitService.getUnits("percent"));
        }
        if (!(_percentSystemTime == null)) {
            final TopologyNode _node81 = currentNode.createNode("percentSystemTime");
            _percentSystemTime.submit(_node81, unitService.getUnits("percent"));
        }
        if (!(_percentIdleTime == null)) {
            final TopologyNode _node82 = currentNode.createNode("percentIdleTime");
            _percentIdleTime.submit(_node82, unitService.getUnits("percent"));
        }
        if (!(_utilization == null)) {
            final TopologyNode _node83 = currentNode.createNode("utilization");
            _utilization.submit(_node83, unitService.getUnits("percent"));
        }
        if (!(_usedHz == null)) {
            final TopologyNode _node84 = currentNode.createNode("usedHz");
            _usedHz.submit(_node84, unitService.getUnits("count"));
        }
        if (!(_totalHz == null)) {
            final TopologyNode _node85 = currentNode.createNode("totalHz");
            _totalHz.submit(_node85, unitService.getUnits("count"));
        }
        if (!(_topCPUProcesses == null)) {
            for (TopologyObject list_element: _topCPUProcesses) {
                final TopologyNode _node86 = currentNode.createNode("topCPUProcesses");
                if (__isIdentity("topCPUProcesses")) {
                    _node86 .setIsIdentity(true);
                }
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node86 .setTypeHint(cloned.__getNodeName());
                    _node86 .setId(cloned.__getNodeId().toString());
                    cloned.addToNodeTree(_node86, unitService, submitted);
                } else {
                    // Not already submitted, send the entire thing
                    _node86 .setTypeHint(list_element.__getNodeName());
                    _node86 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node86, unitService, submitted);
                }
            }
        }
    }
