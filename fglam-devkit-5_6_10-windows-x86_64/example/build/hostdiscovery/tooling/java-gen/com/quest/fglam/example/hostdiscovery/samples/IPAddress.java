    ...
    extends HostBaseObject
{
        ...
        throws TopologyException
    {
        ...
        if (!(_address == null)) {
            final TopologyValue _value10 = currentNode.createValue("address");
            _value10 .setSampleValue(_address.toString());
        ...
        if (!(_originatingPaths == null)) {
            for (TopologyObject list_element: _originatingPaths) {
                final TopologyNode _node11 = currentNode.createNode("originatingPaths");
                if (__isIdentity("originatingPaths")) {
                    _node11 .setIsIdentity(true);
                }
                if (_replace_originatingPaths) {
                    _node11 .setReplace(true);
                }
                ...
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node11 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node11 .setTypeHint(list_element.__getNodeName());
                    _node11 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node11, unitService, submitted);
                }
            }
        }
        if (!(_terminatingPaths == null)) {
            for (TopologyObject list_element: _terminatingPaths) {
                final TopologyNode _node12 = currentNode.createNode("terminatingPaths");
                if (__isIdentity("terminatingPaths")) {
                    _node12 .setIsIdentity(true);
                }
                if (_replace_terminatingPaths) {
                    _node12 .setReplace(true);
                }
                _replace_terminatingPaths = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node12 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node12 .setTypeHint(list_element.__getNodeName());
                    _node12 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node12, unitService, submitted);
                }
            }
        }
        if (!(_host == null)) {
            final TopologyNode _node13 = currentNode.createNode("host");
            if (__isIdentity("host")) {
                _node13 .setIsIdentity(true);
            }
            if (submitted.contains(_host.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node13 .setRefId(_host.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node13 .setTypeHint(_host.__getNodeName());
                _node13 .setId(_host.__getNodeId().toString());
                _host.addToNodeTree(_node13, unitService, submitted);
            }
        }
    }
