    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_totalMemory == null)) {
            final TopologyValue _value14 = currentNode.createValue("totalMemory");
            _value14 .setSampleValue(_totalMemory, unitService.getUnits("megabyte"));
            if (__isIdentity("totalMemory")) {
                _value14 .setIsIdentity(true);
            }
        }
        if (!(_capacity == null)) {
            final TopologyNode _node15 = currentNode.createNode("capacity");
            _capacity.submit(_node15, unitService.getUnits("megabyte"));
        }
        if (!(_consumed == null)) {
            final TopologyNode _node16 = currentNode.createNode("consumed");
            _consumed.submit(_node16, unitService.getUnits("megabyte"));
        }
        if (!(_utilization == null)) {
            final TopologyNode _node17 = currentNode.createNode("utilization");
            _utilization.submit(_node17, unitService.getUnits("percent"));
        }
        if (!(_pageInRate == null)) {
            final TopologyNode _node18 = currentNode.createNode("pageInRate");
            _pageInRate.submit(_node18, unitService.getUnits("count/second"));
        }
        if (!(_pageOutRate == null)) {
            final TopologyNode _node19 = currentNode.createNode("pageOutRate");
            _pageOutRate.submit(_node19, unitService.getUnits("count/second"));
        }
        if (!(_virtualMemorySize == null)) {
            final TopologyNode _node20 = currentNode.createNode("virtualMemorySize");
            _virtualMemorySize.submit(_node20, unitService.getUnits("megabyte"));
        }
        if (!(_nonPagedKernelMemory == null)) {
            final TopologyNode _node21 = currentNode.createNode("nonPagedKernelMemory");
            _nonPagedKernelMemory.submit(_node21, unitService.getUnits("megabyte"));
        }
        if (!(_pagedKernelMemory == null)) {
            final TopologyNode _node22 = currentNode.createNode("pagedKernelMemory");
            _pagedKernelMemory.submit(_node22, unitService.getUnits("megabyte"));
        }
        if (!(_fileSystemCache == null)) {
            final TopologyNode _node23 = currentNode.createNode("fileSystemCache");
            _fileSystemCache.submit(_node23, unitService.getUnits("megabyte"));
        }
        if (!(_topMemoryProcesses == null)) {
            for (TopologyObject list_element: _topMemoryProcesses) {
                final TopologyNode _node24 = currentNode.createNode("topMemoryProcesses");
                ...
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node24 .setTypeHint(cloned.__getNodeName());
                    _node24 .setId(cloned.__getNodeId().toString());
                    cloned.addToNodeTree(_node24, unitService, submitted);
                } else {
                    // Not already submitted, send the entire thing
                    _node24 .setTypeHint(list_element.__getNodeName());
                    _node24 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node24, unitService, submitted);
                }
            }
        }
    }
