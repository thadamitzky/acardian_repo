    ...
    extends TopologyEdge
{
        ...
        throws TopologyException
    {
        ...
        if (!(_source == null)) {
            final TopologyNode _node166 = currentNode.createNode("source");
            if (__isIdentity("source")) {
                _node166 .setIsIdentity(true);
            }
            if (submitted.contains(_source.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node166 .setRefId(_source.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node166 .setTypeHint(_source.__getNodeName());
                _node166 .setId(_source.__getNodeId().toString());
                _source.addToNodeTree(_node166, unitService, submitted);
            }
        }
        if (!(_destination == null)) {
            final TopologyNode _node167 = currentNode.createNode("destination");
            if (__isIdentity("destination")) {
                _node167 .setIsIdentity(true);
            }
            if (submitted.contains(_destination.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node167 .setRefId(_destination.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node167 .setTypeHint(_destination.__getNodeName());
                _node167 .setId(_destination.__getNodeId().toString());
                _destination.addToNodeTree(_node167, unitService, submitted);
            }
        }
    }
