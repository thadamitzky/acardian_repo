    ...
    extends DataObject
{
        ...
        throws TopologyException
    {
        ...
        if (!(_startTime == null)) {
            final TopologyValue _value161 = currentNode.createValue("startTime");
            _value161 .setSampleValue(_startTime);
            if (__isIdentity("startTime")) {
                _value161 .setIsIdentity(true);
            }
        }
        if (!(_endTime == null)) {
            final TopologyValue _value162 = currentNode.createValue("endTime");
            _value162 .setSampleValue(_endTime);
            if (__isIdentity("endTime")) {
                _value162 .setIsIdentity(true);
            }
        }
        if (!(_sampledPeriod == null)) {
            final TopologyValue _value163 = currentNode.createValue("sampledPeriod");
            _value163 .setSampleValue(_sampledPeriod, unitService.getUnits("millisecond"));
            if (__isIdentity("sampledPeriod")) {
                _value163 .setIsIdentity(true);
            }
        }
        if (!(_agentID == null)) {
            final TopologyValue _value164 = currentNode.createValue("agentID");
            _value164 .setSampleValue(_agentID);
            if (__isIdentity("agentID")) {
                _value164 .setIsIdentity(true);
            }
        }
        if (!(_valueSource == null)) {
            final TopologyValue _value165 = currentNode.createValue("valueSource");
            _value165 .setSampleValue(_valueSource.toString());
