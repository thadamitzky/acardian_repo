    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_interfaces == null)) {
            for (TopologyObject list_element: _interfaces) {
                final TopologyNode _node168 = currentNode.createNode("interfaces");
                if (__isIdentity("interfaces")) {
                    _node168 .setIsIdentity(true);
                }
                if (_replace_interfaces) {
                    _node168 .setReplace(true);
                }
                _replace_interfaces = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node168 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node168 .setTypeHint(list_element.__getNodeName());
                    _node168 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node168, unitService, submitted);
                }
            }
        }
        if (!(_utilization == null)) {
            final TopologyNode _node169 = currentNode.createNode("utilization");
            _utilization.submit(_node169, unitService.getUnits("percent"));
        }
        if (!(_packetsSent == null)) {
            final TopologyNode _node170 = currentNode.createNode("packetsSent");
            _packetsSent.submit(_node170, unitService.getUnits("count/second"));
        }
        if (!(_packetsReceived == null)) {
            final TopologyNode _node171 = currentNode.createNode("packetsReceived");
            _packetsReceived.submit(_node171, unitService.getUnits("count/second"));
        }
        if (!(_sendRate == null)) {
            final TopologyNode _node172 = currentNode.createNode("sendRate");
            _sendRate.submit(_node172, unitService.getUnits("bit/second"));
        }
        if (!(_receiveRate == null)) {
            final TopologyNode _node173 = currentNode.createNode("receiveRate");
            _receiveRate.submit(_node173, unitService.getUnits("bit/second"));
        }
        if (!(_transferRate == null)) {
            final TopologyNode _node174 = currentNode.createNode("transferRate");
            _transferRate.submit(_node174, unitService.getUnits("bit/second"));
        }
    }
