    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_name == null)) {
            final TopologyValue _value156 = currentNode.createValue("name");
            _value156 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value156 .setIsIdentity(true);
            }
        }
        if (!(_displayName == null)) {
            final TopologyValue _value157 = currentNode.createValue("displayName");
            _value157 .setSampleValue(_displayName.toString());
            if (__isIdentity("displayName")) {
                _value157 .setIsIdentity(true);
            }
        }
        if (!(_state == null)) {
            final TopologyValue _value158 = currentNode.createValue("state");
            _value158 .setSampleValue(_state.getValue());
            if (__isIdentity("state")) {
                _value158 .setIsIdentity(true);
            }
        }
        if (!(_process == null)) {
            final TopologyNode _node159 = currentNode.createNode("process");
            if (__isIdentity("process")) {
                _node159 .setIsIdentity(true);
            }
            if (submitted.contains(_process.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node159 .setRefId(_process.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node159 .setTypeHint(_process.__getNodeName());
                _node159 .setId(_process.__getNodeId().toString());
                _process.addToNodeTree(_node159, unitService, submitted);
            }
        }
        if (!(_pid == null)) {
            final TopologyValue _value160 = currentNode.createValue("pid");
            _value160 .setSampleValue(_pid.toString());
            if (__isIdentity("pid")) {
                _value160 .setIsIdentity(true);
            }
        }
    }
