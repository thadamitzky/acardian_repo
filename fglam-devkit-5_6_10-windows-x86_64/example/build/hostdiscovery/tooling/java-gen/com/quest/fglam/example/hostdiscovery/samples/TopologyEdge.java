    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value212 = currentNode.createValue("name");
            _value212 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value212 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node213 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node213 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node213 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node213 .setTypeHint(_monitoringAgent.__getNodeName());
                _node213 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node213, unitService, submitted);
            }
        }
    }
