    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value195 = currentNode.createValue("name");
            _value195 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value195 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node196 = currentNode.createNode("monitoringAgent");
            ...
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                ...
                _node196 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node196 .setTypeHint(_monitoringAgent.__getNodeName());
                _node196 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node196, unitService, submitted);
            }
        }
    }
