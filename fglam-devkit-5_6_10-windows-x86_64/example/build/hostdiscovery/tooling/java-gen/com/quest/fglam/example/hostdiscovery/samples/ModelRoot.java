    ...
    extends TopologyObject
{
        ...
        throws TopologyException
    {
        ...
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node60 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node60 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node60 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node60 .setTypeHint(_monitoringAgent.__getNodeName());
                _node60 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node60, unitService, submitted);
            }
        }
    }
