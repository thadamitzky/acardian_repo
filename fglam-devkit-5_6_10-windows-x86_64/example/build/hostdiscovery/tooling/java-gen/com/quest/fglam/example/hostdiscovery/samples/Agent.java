    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value36 = currentNode.createValue("name");
            _value36 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value36 .setIsIdentity(true);
            }
        }
        if (!(_agentVersion == null)) {
            final TopologyValue _value37 = currentNode.createValue("agentVersion");
            _value37 .setSampleValue(_agentVersion.toString());
            if (__isIdentity("agentVersion")) {
                _value37 .setIsIdentity(true);
            }
        }
        if (!(_agentName == null)) {
            final TopologyValue _value38 = currentNode.createValue("agentName");
            _value38 .setSampleValue(_agentName.toString());
            if (__isIdentity("agentName")) {
                _value38 .setIsIdentity(true);
            }
        }
        if (!(_agentInstance == null)) {
            final TopologyValue _value39 = currentNode.createValue("agentInstance");
            _value39 .setSampleValue(_agentInstance.toString());
            if (__isIdentity("agentInstance")) {
                _value39 .setIsIdentity(true);
            }
        }
        if (!(_build == null)) {
            final TopologyValue _value40 = currentNode.createValue("build");
            _value40 .setSampleValue(_build.toString());
            if (__isIdentity("build")) {
                _value40 .setIsIdentity(true);
            }
        }
        if (!(_cpuCount == null)) {
            final TopologyValue _value41 = currentNode.createValue("cpuCount");
            _value41 .setSampleValue(_cpuCount.toString());
            if (__isIdentity("cpuCount")) {
                _value41 .setIsIdentity(true);
            }
        }
        if (!(_hostName == null)) {
            final TopologyValue _value42 = currentNode.createValue("hostName");
            _value42 .setSampleValue(_hostName.toString());
            if (__isIdentity("hostName")) {
                _value42 .setIsIdentity(true);
            }
        }
        if (!(_type == null)) {
            final TopologyValue _value43 = currentNode.createValue("type");
            _value43 .setSampleValue(_type.toString());
            if (__isIdentity("type")) {
                _value43 .setIsIdentity(true);
            }
        }
        if (!(_runningStateObservation == null)) {
            final TopologyValue _value44 = currentNode.createValue("runningStateObservation");
            _value44 .setSampleValue(_runningStateObservation.getValue());
            if (__isIdentity("runningStateObservation")) {
                _value44 .setIsIdentity(true);
            }
        }
        if (!(_monitoredHost == null)) {
            final TopologyNode _node45 = currentNode.createNode("monitoredHost");
            ...
            if (submitted.contains(_monitoredHost.__getNodeId())) {
                ...
                _node45 .setRefId(_monitoredHost.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node45 .setTypeHint(_monitoredHost.__getNodeName());
                _node45 .setId(_monitoredHost.__getNodeId().toString());
                _monitoredHost.addToNodeTree(_node45, unitService, submitted);
            }
        }
