...
public class HostBaseObject
    extends com.quest.fglam.example.hostdiscovery.samples.TopologyObject
{
        ...
        throws TopologyException
    {
        if (!(_detail == null)) {
            for (com.quest.fglam.example.hostdiscovery.samples.TopologyObject list_element: _detail) {
                final TopologyNode _node192 = currentNode.createNode("detail");
                if (__isIdentity("detail")) {
                    _node192 .setIsIdentity(true);
                }
                if (_replace_detail) {
                    _node192 .setReplace(true);
                }
                _replace_detail = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node192 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node192 .setTypeHint(list_element.__getNodeName());
                    _node192 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node192, unitService, submitted);
                }
            }
        }
        if (!(_name == null)) {
            final TopologyValue _value193 = currentNode.createValue("name");
            _value193 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value193 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node194 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node194 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node194 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node194 .setTypeHint(_monitoringAgent.__getNodeName());
                _node194 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node194, unitService, submitted);
            }
        }
    }
