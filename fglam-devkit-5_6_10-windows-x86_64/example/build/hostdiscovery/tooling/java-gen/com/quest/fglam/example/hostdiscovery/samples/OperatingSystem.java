    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_type == null)) {
            final TopologyValue _value25 = currentNode.createValue("type");
            _value25 .setSampleValue(_type.toString());
            if (__isIdentity("type")) {
                _value25 .setIsIdentity(true);
            }
        }
        if (!(_name == null)) {
            final TopologyValue _value26 = currentNode.createValue("name");
            _value26 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value26 .setIsIdentity(true);
            }
        }
        if (!(_architecture == null)) {
            final TopologyValue _value27 = currentNode.createValue("architecture");
            _value27 .setSampleValue(_architecture.toString());
            if (__isIdentity("architecture")) {
                _value27 .setIsIdentity(true);
            }
        }
        if (!(_version == null)) {
            final TopologyValue _value28 = currentNode.createValue("version");
            _value28 .setSampleValue(_version.toString());
            if (__isIdentity("version")) {
                _value28 .setIsIdentity(true);
            }
        }
        if (!(_servicePackMajorVersion == null)) {
            final TopologyValue _value29 = currentNode.createValue("servicePackMajorVersion");
            _value29 .setSampleValue(_servicePackMajorVersion.toString());
            if (__isIdentity("servicePackMajorVersion")) {
                _value29 .setIsIdentity(true);
            }
        }
        if (!(_servicePackMinorVersion == null)) {
            final TopologyValue _value30 = currentNode.createValue("servicePackMinorVersion");
            _value30 .setSampleValue(_servicePackMinorVersion.toString());
            if (__isIdentity("servicePackMinorVersion")) {
                _value30 .setIsIdentity(true);
            }
        }
        if (!(_buildNumber == null)) {
            final TopologyValue _value31 = currentNode.createValue("buildNumber");
            _value31 .setSampleValue(_buildNumber.toString());
        ...
        if (!(_processes == null)) {
            for (TopologyObject list_element: _processes) {
                final TopologyNode _node32 = currentNode.createNode("processes");
                if (__isIdentity("processes")) {
                    _node32 .setIsIdentity(true);
                }
                if (_replace_processes) {
                    _node32 .setReplace(true);
                }
                _replace_processes = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node32 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node32 .setTypeHint(list_element.__getNodeName());
                    _node32 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node32, unitService, submitted);
                }
            }
        }
        if (!(_services == null)) {
            for (TopologyObject list_element: _services) {
                final TopologyNode _node33 = currentNode.createNode("services");
                if (__isIdentity("services")) {
                    _node33 .setIsIdentity(true);
                }
                if (_replace_services) {
                    _node33 .setReplace(true);
                }
                _replace_services = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node33 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
