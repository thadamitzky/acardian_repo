    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_physicalDisks == null)) {
            for (TopologyObject list_element: _physicalDisks) {
                final TopologyNode _node110 = currentNode.createNode("physicalDisks");
                if (__isIdentity("physicalDisks")) {
                    _node110 .setIsIdentity(true);
                }
                if (_replace_physicalDisks) {
                    _node110 .setReplace(true);
                }
                ...
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node110 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node110 .setTypeHint(list_element.__getNodeName());
                    _node110 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node110, unitService, submitted);
                }
            }
        }
        if (!(_logicalDisks == null)) {
            for (TopologyObject list_element: _logicalDisks) {
                final TopologyNode _node111 = currentNode.createNode("logicalDisks");
                if (__isIdentity("logicalDisks")) {
                    _node111 .setIsIdentity(true);
                }
                if (_replace_logicalDisks) {
                    _node111 .setReplace(true);
                }
                _replace_logicalDisks = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node111 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node111 .setTypeHint(list_element.__getNodeName());
                    _node111 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node111, unitService, submitted);
                }
            }
        }
        if (!(_diskUtilization == null)) {
            final TopologyNode _node112 = currentNode.createNode("diskUtilization");
            _diskUtilization.submit(_node112, unitService.getUnits("percent"));
        }
        if (!(_diskReads == null)) {
            final TopologyNode _node113 = currentNode.createNode("diskReads");
            _diskReads.submit(_node113, unitService.getUnits("count/second"));
        }
        if (!(_diskWrites == null)) {
            final TopologyNode _node114 = currentNode.createNode("diskWrites");
            _diskWrites.submit(_node114, unitService.getUnits("count/second"));
        }
        if (!(_diskBytesRead == null)) {
            final TopologyNode _node115 = currentNode.createNode("diskBytesRead");
            _diskBytesRead.submit(_node115, unitService.getUnits("kilobyte/second"));
        }
        if (!(_diskBytesWritten == null)) {
            final TopologyNode _node116 = currentNode.createNode("diskBytesWritten");
            _diskBytesWritten.submit(_node116, unitService.getUnits("kilobyte/second"));
        }
        if (!(_diskTransferRate == null)) {
            final TopologyNode _node117 = currentNode.createNode("diskTransferRate");
            _diskTransferRate.submit(_node117, unitService.getUnits("kilobyte/second"));
        }
        if (!(_spaceUsed == null)) {
            final TopologyNode _node118 = currentNode.createNode("spaceUsed");
            _spaceUsed.submit(_node118, unitService.getUnits("megabyte"));
        }
        if (!(_spaceAvailable == null)) {
            final TopologyNode _node119 = currentNode.createNode("spaceAvailable");
            _spaceAvailable.submit(_node119, unitService.getUnits("megabyte"));
        }
        if (!(_topIOProcesses == null)) {
            for (TopologyObject list_element: _topIOProcesses) {
                final TopologyNode _node120 = currentNode.createNode("topIOProcesses");
                if (__isIdentity("topIOProcesses")) {
                    _node120 .setIsIdentity(true);
                }
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node120 .setTypeHint(cloned.__getNodeName());
                    _node120 .setId(cloned.__getNodeId().toString());
                    cloned.addToNodeTree(_node120, unitService, submitted);
                } else {
                    // Not already submitted, send the entire thing
                    _node120 .setTypeHint(list_element.__getNodeName());
                    _node120 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node120, unitService, submitted);
                }
            }
        }
    }
