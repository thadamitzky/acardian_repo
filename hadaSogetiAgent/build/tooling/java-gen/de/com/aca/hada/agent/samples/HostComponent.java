    ...
    extends HostBaseObject
{
        ...
        throws TopologyException
    {
        ...
        if (!(_host == null)) {
            final TopologyNode _node109 = currentNode.createNode("host");
            if (__isIdentity("host")) {
                _node109 .setIsIdentity(true);
            }
            if (submitted.contains(_host.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node109 .setRefId(_host.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node109 .setTypeHint(_host.__getNodeName());
                _node109 .setId(_host.__getNodeId().toString());
                _host.addToNodeTree(_node109, unitService, submitted);
            }
        }
    }
