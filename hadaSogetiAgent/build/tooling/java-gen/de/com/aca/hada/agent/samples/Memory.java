    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_totalMemory == null)) {
            final TopologyValue _value83 = currentNode.createValue("totalMemory");
            _value83 .setSampleValue(_totalMemory, unitService.getUnits("megabyte"));
            if (__isIdentity("totalMemory")) {
                _value83 .setIsIdentity(true);
            }
        }
        if (!(_capacity == null)) {
            final TopologyNode _node84 = currentNode.createNode("capacity");
            _capacity.submit(_node84, unitService.getUnits("megabyte"));
        }
        if (!(_consumed == null)) {
            final TopologyNode _node85 = currentNode.createNode("consumed");
            _consumed.submit(_node85, unitService.getUnits("megabyte"));
        }
        if (!(_utilization == null)) {
            final TopologyNode _node86 = currentNode.createNode("utilization");
            _utilization.submit(_node86, unitService.getUnits("percent"));
        }
        if (!(_pageInRate == null)) {
            final TopologyNode _node87 = currentNode.createNode("pageInRate");
            _pageInRate.submit(_node87, unitService.getUnits("count/second"));
        }
        if (!(_pageOutRate == null)) {
            final TopologyNode _node88 = currentNode.createNode("pageOutRate");
            _pageOutRate.submit(_node88, unitService.getUnits("count/second"));
        }
        if (!(_virtualMemorySize == null)) {
            final TopologyNode _node89 = currentNode.createNode("virtualMemorySize");
            _virtualMemorySize.submit(_node89, unitService.getUnits("megabyte"));
        }
        if (!(_nonPagedKernelMemory == null)) {
            final TopologyNode _node90 = currentNode.createNode("nonPagedKernelMemory");
            _nonPagedKernelMemory.submit(_node90, unitService.getUnits("megabyte"));
        }
        if (!(_pagedKernelMemory == null)) {
            final TopologyNode _node91 = currentNode.createNode("pagedKernelMemory");
            _pagedKernelMemory.submit(_node91, unitService.getUnits("megabyte"));
        }
        if (!(_fileSystemCache == null)) {
            final TopologyNode _node92 = currentNode.createNode("fileSystemCache");
            _fileSystemCache.submit(_node92, unitService.getUnits("megabyte"));
        }
        if (!(_topMemoryProcesses == null)) {
            for (TopologyObject list_element: _topMemoryProcesses) {
                final TopologyNode _node93 = currentNode.createNode("topMemoryProcesses");
                if (__isIdentity("topMemoryProcesses")) {
                    _node93 .setIsIdentity(true);
                }
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node93 .setTypeHint(cloned.__getNodeName());
                    _node93 .setId(cloned.__getNodeId().toString());
                    cloned.addToNodeTree(_node93, unitService, submitted);
                } else {
                    // Not already submitted, send the entire thing
                    _node93 .setTypeHint(list_element.__getNodeName());
                    _node93 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node93, unitService, submitted);
                }
            }
        }
    }
