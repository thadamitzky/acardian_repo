    ...
    extends DataObject
{
        ...
        throws TopologyException
    {
        ...
        if (!(_processId == null)) {
            final TopologyValue _value105 = currentNode.createValue("processId");
            _value105 .setSampleValue(_processId.toString());
            if (__isIdentity("processId")) {
                _value105 .setIsIdentity(true);
            }
        }
        if (!(_name == null)) {
            final TopologyValue _value106 = currentNode.createValue("name");
            _value106 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value106 .setIsIdentity(true);
            }
        }
        if (!(_userName == null)) {
            final TopologyValue _value107 = currentNode.createValue("userName");
            _value107 .setSampleValue(_userName.toString());
            if (__isIdentity("userName")) {
                _value107 .setIsIdentity(true);
            }
        }
        if (!(_command == null)) {
            final TopologyValue _value108 = currentNode.createValue("command");
            _value108 .setSampleValue(_command.toString());
