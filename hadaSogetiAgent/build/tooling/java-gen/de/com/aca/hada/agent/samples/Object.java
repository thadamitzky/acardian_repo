    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value225 = currentNode.createValue("name");
            _value225 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value225 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node226 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node226 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node226 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node226 .setTypeHint(_monitoringAgent.__getNodeName());
                _node226 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node226, unitService, submitted);
            }
        }
    }
