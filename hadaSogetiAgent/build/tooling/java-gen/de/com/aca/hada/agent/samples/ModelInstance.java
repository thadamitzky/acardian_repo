    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value77 = currentNode.createValue("name");
            _value77 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value77 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node78 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node78 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node78 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node78 .setTypeHint(_monitoringAgent.__getNodeName());
                _node78 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node78, unitService, submitted);
            }
        }
    }
