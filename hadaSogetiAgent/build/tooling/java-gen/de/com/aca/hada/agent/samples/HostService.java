    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_name == null)) {
            final TopologyValue _value154 = currentNode.createValue("name");
            _value154 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value154 .setIsIdentity(true);
            }
        }
        if (!(_displayName == null)) {
            final TopologyValue _value155 = currentNode.createValue("displayName");
            _value155 .setSampleValue(_displayName.toString());
            if (__isIdentity("displayName")) {
                _value155 .setIsIdentity(true);
            }
        }
        if (!(_state == null)) {
            final TopologyValue _value156 = currentNode.createValue("state");
            _value156 .setSampleValue(_state.getValue());
            if (__isIdentity("state")) {
                _value156 .setIsIdentity(true);
            }
        }
        if (!(_process == null)) {
            final TopologyNode _node157 = currentNode.createNode("process");
            if (__isIdentity("process")) {
                _node157 .setIsIdentity(true);
            }
            if (submitted.contains(_process.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node157 .setRefId(_process.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node157 .setTypeHint(_process.__getNodeName());
                _node157 .setId(_process.__getNodeId().toString());
                _process.addToNodeTree(_node157, unitService, submitted);
            }
        }
        if (!(_pid == null)) {
            final TopologyValue _value158 = currentNode.createValue("pid");
            _value158 .setSampleValue(_pid.toString());
            if (__isIdentity("pid")) {
