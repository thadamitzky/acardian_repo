    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value30 = currentNode.createValue("name");
            _value30 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value30 .setIsIdentity(true);
            }
        }
        if (!(_agentVersion == null)) {
            final TopologyValue _value31 = currentNode.createValue("agentVersion");
            _value31 .setSampleValue(_agentVersion.toString());
            if (__isIdentity("agentVersion")) {
                _value31 .setIsIdentity(true);
            }
        }
        if (!(_agentName == null)) {
            final TopologyValue _value32 = currentNode.createValue("agentName");
            _value32 .setSampleValue(_agentName.toString());
            if (__isIdentity("agentName")) {
                _value32 .setIsIdentity(true);
            }
        }
        if (!(_agentInstance == null)) {
            final TopologyValue _value33 = currentNode.createValue("agentInstance");
            _value33 .setSampleValue(_agentInstance.toString());
            if (__isIdentity("agentInstance")) {
                _value33 .setIsIdentity(true);
            }
        }
        if (!(_build == null)) {
            final TopologyValue _value34 = currentNode.createValue("build");
            _value34 .setSampleValue(_build.toString());
            if (__isIdentity("build")) {
                _value34 .setIsIdentity(true);
            }
        }
        if (!(_cpuCount == null)) {
            final TopologyValue _value35 = currentNode.createValue("cpuCount");
            _value35 .setSampleValue(_cpuCount.toString());
            if (__isIdentity("cpuCount")) {
                _value35 .setIsIdentity(true);
            }
        }
        if (!(_hostName == null)) {
            final TopologyValue _value36 = currentNode.createValue("hostName");
            _value36 .setSampleValue(_hostName.toString());
            if (__isIdentity("hostName")) {
                _value36 .setIsIdentity(true);
            }
        }
        if (!(_type == null)) {
            final TopologyValue _value37 = currentNode.createValue("type");
            _value37 .setSampleValue(_type.toString());
            if (__isIdentity("type")) {
                _value37 .setIsIdentity(true);
            }
        }
        if (!(_runningStateObservation == null)) {
            final TopologyValue _value38 = currentNode.createValue("runningStateObservation");
            _value38 .setSampleValue(_runningStateObservation.getValue());
            if (__isIdentity("runningStateObservation")) {
                _value38 .setIsIdentity(true);
            }
        }
        if (!(_monitoredHost == null)) {
            final TopologyNode _node39 = currentNode.createNode("monitoredHost");
            if (__isIdentity("monitoredHost")) {
                _node39 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoredHost.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node39 .setRefId(_monitoredHost.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node39 .setTypeHint(_monitoredHost.__getNodeName());
                _node39 .setId(_monitoredHost.__getNodeId().toString());
                _monitoredHost.addToNodeTree(_node39, unitService, submitted);
            }
        }
