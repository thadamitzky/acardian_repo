...
public class HostBaseObject
    extends de.com.aca.hada.agent.samples.TopologyObject
{
        ...
        throws TopologyException
    {
        if (!(_detail == null)) {
            for (de.com.aca.hada.agent.samples.TopologyObject list_element: _detail) {
                final TopologyNode _node178 = currentNode.createNode("detail");
                if (__isIdentity("detail")) {
                    _node178 .setIsIdentity(true);
                }
                if (_replace_detail) {
                    _node178 .setReplace(true);
                }
                _replace_detail = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node178 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node178 .setTypeHint(list_element.__getNodeName());
                    _node178 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node178, unitService, submitted);
                }
            }
        }
        if (!(_name == null)) {
            final TopologyValue _value179 = currentNode.createValue("name");
            _value179 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value179 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node180 = currentNode.createNode("monitoringAgent");
            if (__isIdentity("monitoringAgent")) {
                _node180 .setIsIdentity(true);
            }
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node180 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node180 .setTypeHint(_monitoringAgent.__getNodeName());
                _node180 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node180, unitService, submitted);
            }
        }
    }
