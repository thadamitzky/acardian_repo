    ...
    extends HostBaseObject
{
        ...
        throws TopologyException
    {
        ...
        if (!(_address == null)) {
            final TopologyValue _value79 = currentNode.createValue("address");
            _value79 .setSampleValue(_address.toString());
        ...
        if (!(_originatingPaths == null)) {
            for (TopologyObject list_element: _originatingPaths) {
                final TopologyNode _node80 = currentNode.createNode("originatingPaths");
                if (__isIdentity("originatingPaths")) {
                    _node80 .setIsIdentity(true);
                }
                if (_replace_originatingPaths) {
                    _node80 .setReplace(true);
                }
                ...
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node80 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node80 .setTypeHint(list_element.__getNodeName());
                    _node80 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node80, unitService, submitted);
                }
            }
        }
        if (!(_terminatingPaths == null)) {
            for (TopologyObject list_element: _terminatingPaths) {
                final TopologyNode _node81 = currentNode.createNode("terminatingPaths");
                if (__isIdentity("terminatingPaths")) {
                    _node81 .setIsIdentity(true);
                }
                if (_replace_terminatingPaths) {
                    _node81 .setReplace(true);
                }
                _replace_terminatingPaths = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node81 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node81 .setTypeHint(list_element.__getNodeName());
                    _node81 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node81, unitService, submitted);
                }
            }
        }
        if (!(_host == null)) {
            final TopologyNode _node82 = currentNode.createNode("host");
            if (__isIdentity("host")) {
                _node82 .setIsIdentity(true);
            }
            if (submitted.contains(_host.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node82 .setRefId(_host.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node82 .setTypeHint(_host.__getNodeName());
                _node82 .setId(_host.__getNodeId().toString());
                _host.addToNodeTree(_node82, unitService, submitted);
            }
        }
    }
