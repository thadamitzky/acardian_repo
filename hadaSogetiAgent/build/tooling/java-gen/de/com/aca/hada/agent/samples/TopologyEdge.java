    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value196 = currentNode.createValue("name");
            _value196 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value196 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node197 = currentNode.createNode("monitoringAgent");
            ...
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                ...
                _node197 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node197 .setTypeHint(_monitoringAgent.__getNodeName());
                _node197 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node197, unitService, submitted);
            }
        }
    }
