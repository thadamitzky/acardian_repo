    ...
    extends TopologyEdge
{
        ...
        throws TopologyException
    {
        ...
        if (!(_source == null)) {
            final TopologyNode _node159 = currentNode.createNode("source");
            ...
            if (submitted.contains(_source.__getNodeId())) {
                ...
                _node159 .setRefId(_source.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node159 .setTypeHint(_source.__getNodeName());
                _node159 .setId(_source.__getNodeId().toString());
                _source.addToNodeTree(_node159, unitService, submitted);
            }
        }
        if (!(_destination == null)) {
            final TopologyNode _node160 = currentNode.createNode("destination");
            if (__isIdentity("destination")) {
                _node160 .setIsIdentity(true);
            }
            if (submitted.contains(_destination.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node160 .setRefId(_destination.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node160 .setTypeHint(_destination.__getNodeName());
                _node160 .setId(_destination.__getNodeId().toString());
                _destination.addToNodeTree(_node160, unitService, submitted);
            }
        }
    }
