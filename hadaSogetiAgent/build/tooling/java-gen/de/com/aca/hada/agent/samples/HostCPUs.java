    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_processors == null)) {
            for (TopologyObject list_element: _processors) {
                final TopologyNode _node110 = currentNode.createNode("processors");
        ...
        if (!(_percentUserTime == null)) {
            final TopologyNode _node111 = currentNode.createNode("percentUserTime");
            _percentUserTime.submit(_node111, unitService.getUnits("percent"));
        }
        if (!(_percentSystemTime == null)) {
            final TopologyNode _node112 = currentNode.createNode("percentSystemTime");
            _percentSystemTime.submit(_node112, unitService.getUnits("percent"));
        }
        if (!(_percentIdleTime == null)) {
            final TopologyNode _node113 = currentNode.createNode("percentIdleTime");
            _percentIdleTime.submit(_node113, unitService.getUnits("percent"));
        }
        if (!(_utilization == null)) {
            final TopologyNode _node114 = currentNode.createNode("utilization");
            _utilization.submit(_node114, unitService.getUnits("percent"));
        }
        if (!(_usedHz == null)) {
            final TopologyNode _node115 = currentNode.createNode("usedHz");
            _usedHz.submit(_node115, unitService.getUnits("count"));
        }
        if (!(_totalHz == null)) {
            final TopologyNode _node116 = currentNode.createNode("totalHz");
            _totalHz.submit(_node116, unitService.getUnits("count"));
        }
        if (!(_topCPUProcesses == null)) {
            for (TopologyObject list_element: _topCPUProcesses) {
                final TopologyNode _node117 = currentNode.createNode("topCPUProcesses");
                if (__isIdentity("topCPUProcesses")) {
                    _node117 .setIsIdentity(true);
                }
                if (submitted.contains(list_element.__getNodeId())) {
                    ...
                    _node117 .setTypeHint(cloned.__getNodeName());
                    _node117 .setId(cloned.__getNodeId().toString());
                    cloned.addToNodeTree(_node117, unitService, submitted);
                } else {
                    // Not already submitted, send the entire thing
                    _node117 .setTypeHint(list_element.__getNodeName());
                    _node117 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node117, unitService, submitted);
                }
            }
        }
    }
