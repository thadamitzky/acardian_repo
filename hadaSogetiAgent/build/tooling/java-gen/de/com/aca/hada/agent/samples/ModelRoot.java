    ...
    extends TopologyObject
{
    ...
    {
        if (!(_name == null)) {
            final TopologyValue _value40 = currentNode.createValue("name");
            _value40 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value40 .setIsIdentity(true);
            }
        }
        if (!(_monitoringAgent == null)) {
            final TopologyNode _node41 = currentNode.createNode("monitoringAgent");
            ...
            if (submitted.contains(_monitoringAgent.__getNodeId())) {
                ...
                _node41 .setRefId(_monitoringAgent.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node41 .setTypeHint(_monitoringAgent.__getNodeName());
                _node41 .setId(_monitoringAgent.__getNodeId().toString());
                _monitoringAgent.addToNodeTree(_node41, unitService, submitted);
            }
        }
    }
