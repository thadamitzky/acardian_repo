    ...
    extends HostComponent
{
        ...
        throws TopologyException
    {
        ...
        if (!(_name == null)) {
            final TopologyValue _value118 = currentNode.createValue("name");
            _value118 .setSampleValue(_name.toString());
            if (__isIdentity("name")) {
                _value118 .setIsIdentity(true);
            }
        }
        if (!(_ipAddress == null)) {
            final TopologyNode _node119 = currentNode.createNode("ipAddress");
            if (__isIdentity("ipAddress")) {
                _node119 .setIsIdentity(true);
            }
            if (submitted.contains(_ipAddress.__getNodeId())) {
                // field already submitted, just set the reference ID
                _node119 .setRefId(_ipAddress.__getNodeId().toString());
            } else {
                // Not already submitted, send the entire thing
                _node119 .setTypeHint(_ipAddress.__getNodeName());
                _node119 .setId(_ipAddress.__getNodeId().toString());
                _ipAddress.addToNodeTree(_node119, unitService, submitted);
            }
        }
        if (!(_macAddress == null)) {
            final TopologyValue _value120 = currentNode.createValue("macAddress");
            _value120 .setSampleValue(_macAddress.toString());
            if (__isIdentity("macAddress")) {
                _value120 .setIsIdentity(true);
            }
        }
        if (!(_interfaceIndex == null)) {
            final TopologyValue _value121 = currentNode.createValue("interfaceIndex");
            _value121 .setSampleValue(_interfaceIndex);
            if (__isIdentity("interfaceIndex")) {
                _value121 .setIsIdentity(true);
            }
        }
        if (!(_portNumber == null)) {
            final TopologyValue _value122 = currentNode.createValue("portNumber");
            _value122 .setSampleValue(_portNumber);
            if (__isIdentity("portNumber")) {
                _value122 .setIsIdentity(true);
            }
        }
        if (!(_interfaceType == null)) {
            final TopologyValue _value123 = currentNode.createValue("interfaceType");
            _value123 .setSampleValue(_interfaceType.toString());
            if (__isIdentity("interfaceType")) {
                _value123 .setIsIdentity(true);
            }
        }
        if (!(_netmask == null)) {
            final TopologyValue _value124 = currentNode.createValue("netmask");
            _value124 .setSampleValue(_netmask.toString());
            if (__isIdentity("netmask")) {
                _value124 .setIsIdentity(true);
            }
        }
        if (!(_isLayer2 == null)) {
            final TopologyValue _value125 = currentNode.createValue("isLayer2");
            _value125 .setSampleValue(_isLayer2 .toString());
            if (__isIdentity("isLayer2")) {
                _value125 .setIsIdentity(true);
            }
        }
        if (!(_networkSegment == null)) {
            final TopologyValue _value126 = currentNode.createValue("networkSegment");
            _value126 .setSampleValue(_networkSegment.toString());
            if (__isIdentity("networkSegment")) {
                _value126 .setIsIdentity(true);
            }
        }
        if (!(_layer2Segment == null)) {
            final TopologyValue _value127 = currentNode.createValue("layer2Segment");
            _value127 .setSampleValue(_layer2Segment);
            if (__isIdentity("layer2Segment")) {
                _value127 .setIsIdentity(true);
            }
        }
        if (!(_discoveryStatus == null)) {
            final TopologyValue _value128 = currentNode.createValue("discoveryStatus");
            _value128 .setSampleValue(_discoveryStatus.getValue());
            if (__isIdentity("discoveryStatus")) {
                _value128 .setIsIdentity(true);
            }
        }
        if (!(_networks == null)) {
            for (TopologyObject list_element: _networks) {
                final TopologyNode _node129 = currentNode.createNode("networks");
                if (__isIdentity("networks")) {
                    _node129 .setIsIdentity(true);
                }
                if (_replace_networks) {
                    _node129 .setReplace(true);
                }
                _replace_networks = false;
                if (submitted.contains(list_element.__getNodeId())) {
                    // field already submitted, just set the reference ID
                    _node129 .setRefId(list_element.__getNodeId().toString());
                } else {
                    // Not already submitted, send the entire thing
                    _node129 .setTypeHint(list_element.__getNodeName());
                    _node129 .setId(list_element.__getNodeId().toString());
                    list_element.addToNodeTree(_node129, unitService, submitted);
                }
            }
        }
        if (!(_bandwidth == null)) {
            final TopologyNode _node130 = currentNode.createNode("bandwidth");
            _bandwidth.submit(_node130, unitService.getUnits("bit/second"));
        }
        if (!(_utilization == null)) {
            final TopologyNode _node131 = currentNode.createNode("utilization");
            _utilization.submit(_node131, unitService.getUnits("percent"));
        }
        if (!(_packetsSent == null)) {
            final TopologyNode _node132 = currentNode.createNode("packetsSent");
            _packetsSent.submit(_node132, unitService.getUnits("count/second"));
        }
        if (!(_packetsReceived == null)) {
            final TopologyNode _node133 = currentNode.createNode("packetsReceived");
            _packetsReceived.submit(_node133, unitService.getUnits("count/second"));
        }
        if (!(_sendRate == null)) {
            final TopologyNode _node134 = currentNode.createNode("sendRate");
            _sendRate.submit(_node134, unitService.getUnits("bit/second"));
        }
        if (!(_receiveRate == null)) {
            final TopologyNode _node135 = currentNode.createNode("receiveRate");
            _receiveRate.submit(_node135, unitService.getUnits("bit/second"));
        }
        if (!(_inboundPacketsDropped == null)) {
            final TopologyNode _node136 = currentNode.createNode("inboundPacketsDropped");
            _inboundPacketsDropped.submit(_node136, unitService.getUnits("count"));
        }
        if (!(_outboundPacketsDropped == null)) {
            final TopologyNode _node137 = currentNode.createNode("outboundPacketsDropped");
            _outboundPacketsDropped.submit(_node137, unitService.getUnits("count"));
        }
        if (!(_inboundErrors == null)) {
            final TopologyNode _node138 = currentNode.createNode("inboundErrors");
            _inboundErrors.submit(_node138, unitService.getUnits("count"));
        }
        if (!(_outboundErrors == null)) {
            final TopologyNode _node139 = currentNode.createNode("outboundErrors");
            _outboundErrors.submit(_node139, unitService.getUnits("count"));
        }
        if (!(_outputQueueLength == null)) {
            final TopologyNode _node140 = currentNode.createNode("outputQueueLength");
            _outputQueueLength.submit(_node140, unitService.getUnits("count"));
        }
    }
